...
export class ShipmentService {
  ...
  /**
   * Khách từ chối — xử lý 2 lý do với logic tồn kho khác nhau:
   *
   * Lý do 1 "Khách không nhận hàng": hoàn về kho gốc, CỘNG lại tồn
  ...
  async customerReject(salesOrderId: string, reason: string) {
    ...
    // Chuẩn hóa lý do
    const REJECT_CUSTOMER_REFUSED = 'Khách không nhận hàng';
    const REJECT_SHIP_DAMAGE = 'Hư hỏng do Vận chuyển';

    let finalReason = reason;
    if (reason.includes('bể') || reason.includes('vỡ') || reason.includes('hư') || reason.includes('vận chuyển') || reason.includes('ship')) {
      finalReason = REJECT_SHIP_DAMAGE;
    } else {
      finalReason = REJECT_CUSTOMER_REFUSED;
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // ── LÝ DO 1: Khách không nhận → trả về kho gốc + CỘNG tồn ──
      if (finalReason === REJECT_CUSTOMER_REFUSED) {
        if (warehouseId) {
          for (const item of order.items) {
            ...
            await tx.inventoryTransaction.create({
              data: {
                ...
                note: `Hoàn trả do khách không nhận hàng - Đơn ${order.orderNo}`,
              },
            });
          }
        }
        ...
        await notificationService.create({
          ...
          title: 'Khách không nhận - Hoàn trả kho',
          message: `Đơn ${order.orderNo}: Khách không nhận hàng. Hàng đã hoàn trả về kho gốc.`,
        });
      }

      // ── LÝ DO 2: Hư hỏng do vận chuyển → KHÔNG cộng tồn ──
      else {
